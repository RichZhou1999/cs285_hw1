...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            max_path_length = len(paths[0]['observation'])
        else:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor, params['batch_size'], max_path_length)
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    ...
                    labeled_actions = expert_policy.get_action(observations)
                    paths[i]["action"] = copy.deepcopy(labeled_actions)
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          batch_num = params['train_batch_size']
          random_indices_path = np.random.permutation(len(paths))
          length = min(batch_num, len(paths[random_indices_path[0]]['observation']))
          ob_batch = []
          ac_batch = []
          random_indices = np.random.permutation(len(paths[random_indices_path[0]]['observation']))
          for i in range(length):
            ob_batch.append(paths[random_indices_path[0]]['observation'][random_indices[i]])
            ac_batch.append(paths[random_indices_path[0]]['action'][random_indices[i]])
